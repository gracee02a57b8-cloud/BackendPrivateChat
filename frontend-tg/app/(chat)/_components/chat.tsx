...
const Chat: FC<Props> = ({ onSubmitMessage, messageForm, messages, onReadMessages, onDeleteMessage, onTyping }) => {
	...
	useEffect(() => {
		if (editedMessage) {
			messageForm.setValue('text', editedMessage.content)
			scrollRef.current?.scrollIntoView({ behavior: 'smooth' })
		}
	}, [editedMessage])
	...
	return (
		<div className='flex flex-col justify-end z-40 min-h-[92vh] sidebar-custom-scrollbar overflow-y-scroll'>
			...
			{messages.map((message, index) => (
				<MessageCard key={message.id || index} message={message} onDeleteMessage={onDeleteMessage} />
			))}

			{/* Start conversation */}
			{messages.length === 0 && !loadMessages && (
