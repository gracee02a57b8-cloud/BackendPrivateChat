...
- ↩️ Ответы на сообщения с уведомлениями
...
### ✅ Задачи
- Канбан-доска
- Уведомления о задачах (созданные, завершённые, просроченные)

### 🛡️ Админ-панель
- Всего пользователей
- Пользователей онлайн (real-time WebSocket)
- Всего чатов (личные + групповые)
- Активных чатов за 24 часа
- Количество групповых чатов
- Количество личных чатов (1-1)
...
│   │   ├── JwtService               # JWT с role claim
│   │   ├── KeyBundleService         # E2E ключи (X3DH)
│   │   ├── NewsService              # Новости + комментарии
│   │   ├── RoomService              # Комнаты + участники
...
│   │   ├── NewsEntity               # Новость
│   │   ├── NewsCommentEntity        # Комментарий к новости
│   │   └── TaskEntity               # Задача
│   ├── config/                  # Конфигурации
...
| Метод | Путь | Описание |
|---|---|---|
| POST | `/register` | Регистрация (возвращает token + username + role) |
| POST | `/login` | Вход (возвращает token + username + role) |
...
|---|---|---|
| GET | `/` | Мои комнаты |
| POST | `/private/{username}` | Создать / получить личный чат |
| POST | `/create` | Создать групповую комнату |
| POST | `/join/{roomId}` | Войти в комнату |
| GET | `/{roomId}` | Информация о комнате |
| GET | `/{roomId}/history` | История сообщений (с пагинацией) |
| DELETE | `/{roomId}` | Удалить комнату (владелец) |

### E2E ключи — Signal Protocol (`/api/keys`)
| Метод | Путь | Описание |
|---|---|---|
| POST | `/bundle` | Загрузить Key Bundle |
| GET | `/bundle/{username}` | Получить ключи (потребляет 1 OTK) |
| POST | `/replenish` | Пополнить One-Time Pre-Keys |
| GET | `/count` | Количество оставшихся OTK |
| GET | `/identity/me` | Свой identity key (верификация) |
| GET | `/identity/{username}` | Identity key пользователя |
| GET | `/has-bundle/{username}` | Есть ли E2E у пользователя |
...
| Метод | Путь | Описание |
|---|---|---|
| POST | `/api/upload` | Загрузить файл (до 100 МБ) |
...
| GET | `/{id}/comments` | Комментарии к новости |
